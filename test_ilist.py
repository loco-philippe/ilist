...
class Test_ilist(unittest.TestCase):
    ...
    def test_creation_list(self) :
        self.assertEqual(Ilist.Ilist([[1,2], [0,1], [1,2]], [0, 1, 2]).sort(inplace=False).iidx, 
                         Ilist.Ilist(['b', 'a', 'b'], [0, 1, 2]).sort(inplace=False).iidx)
        
    def test_creation(self) :
        self.assertEqual(Ilist.Ilist(['a', 'b', 'c']), Ilist.Ilist(['a', 'b', 'c'], [0, 1, 2]))        
        self.assertEqual(Ilist.Ilist(extidx=[1, 2, 3]), Ilist.Ilist([True, True, True], [1, 2, 3]))
        self.assertEqual(Ilist.Ilist(extidx=[[1, 2, 3]]), Ilist.Ilist([True, True, True], [1, 2, 3]))
    ...
    def test_init_index(self) :
        ...
        idxref = [0,0,2]
        extval = ['a', 'b', 'c', 'd', 'e', 'f']  
        self.assertEqual( Ilist.Iindex(extval, idxlen, idxref, order).idxlen, idxlen)
        self.assertEqual( Ilist.Iindex(extval, idxlen, idxref, order).idxref, idxref)
        idxlen = [2,3,3]       
        idxref = [0,1,1]
        self.assertEqual( Ilist.Iindex(extval, idxlen, idxref, order).idxlen, idxlen)
        self.assertEqual( Ilist.Iindex(extval, idxlen, idxref, order).idxref, idxref)
        ...
        idxref = [0,0,0]
        extval = ['a', 'b', 'c']  
        self.assertEqual( Ilist.Iindex(extval, idxlen, idxref, order).iidx[1], [0, 1, 2])
        
    def test_properties(self) :
        l = [[0, 2, 0, 2], [30, 12, 20, 15], [2, 0, 2, 0], [2, 2, 0, 0], ['info', 'info', 'info', 'info'],[12, 20, 15, 30]]
        il=Ilist.Ilist(f,l)
        self.assertEqual( il.idxref, [0, 1, 0, 3, 4, 1])
        self.assertEqual( il.idxlen, [2, 4, 2, 2, 1, 4])
        self.assertEqual( il.dimension, 3)
        self.assertEqual( il.lencomplete, 16)    
    ...
    def test_add_update_list(self) :  
        il=Ilist.Ilist([1,2,3])
        il.addlist('test', [0,1,1])
        self.assertEqual(il.extidx[0], [0,1,1])
        il.updatelist([0,2,2],0)
        self.assertEqual(il.extidx[0], [0,2,2])

    def test_add(self):
        il1 = Ilist.Ilist(['er', 'rt', 'er', 'ry', 'ab'], [[0, 2, 0, 2, 0], [10,0,20,20,15]])
        il2 = Ilist.Ilist(['_er', '_rt', '_er', '_ry', '_ab'], [[10, 12, 10, 12, 10], [110,10,120,120,115]])
        il3 = il1 + il2
        ...
        il2=Ilist.Ilist(['_er', '_rt', '_er', '_ry', '_ab'], [[10, 2, 10, 12, 10], [110,0,120,120,115]])
        il3 = il1 + il2
    ...
    def test_extend(self):
        ...
        il=Ilist.Ilist(['er', 'rt', 'er', 'ry', 'ab', 'ert'])
        ilx = Ilist.Ilist(extidx=[[0,0,0,1,1,1], [0,1,2,3,4,1]])
        il2 = il | ilx
    ...
    def test_extend(self):
        ...
        il2.addlist('truc2', ['un', 'de', 'un', 'de', 'un'])
    ...
    def test_to_xarray(self):
        il=Ilist.Ilist(['er', 'rt', 'er', 'ry', 'ab'], [[0, 2, 0, 2, 0], [10,0,20,20,15], [1,2,1,2,1]])
        self.assertTrue(str(il.to_xarray().sel(idx0=2, idx1=20).values), il.loc([2,20,2]))
        self.assertTrue(str(il.to_xarray(ind='all').sel(idx0=2, idx1=20, idx2=2).values), il.loc([2,20,2]))

    def test_csv(self):
        il=Ilist.Ilist(['er', 'rt', 'er', 'ry', 'ab'], [[0, 2, 0, 2, 0], [10,0,20,20,15], [1,2,1,2,1]])
        il.to_csv('ilist.txt', dialect='excel')
        il2 = Ilist.from_csv('ilist.txt')
        self.assertEqual(il, il2)
    ...
    def test_zip(self):
        il = Ilist.Zip([1, 2, 3], [10, 20, 30], [100, 200, 300])
